...
public class ChatMessageFacadeServiceImpl implements ChatMessageFacadeService {
    ...
    private ChatMessage doCreateChatMessage(ChatMessageDTO chatMessageDTO) {
        try {
            ...
            // 插入数据库，ID 由数据库自动生成, 前端来的request 中自带role属性，user
     ...
     * @return 包含消息ID的响应对象
     * @throws BizException 消息不存在或追加失败时抛出
     */
    @Override
     ...
     * 删除聊天消息
     * @param chatMessageId 消息ID
     * @throws BizException 消息不存在或删除失败时抛出
     */
    @Override
     ...
     * 更新聊天消息
     * @param chatMessageId 消息ID
     * @param request 更新消息请求
     * @throws BizException 消息不存在或更新失败时抛出
     */
    @Override
