...
import static com.heima.tool.Constants.PDF_MEMORY_KEY;

@Repository
public class RedisPDFMemory implements ChatMemory {

    private StringRedisTemplate redisTemplate;
    private ObjectMapper objectMapper;

    public RedisPDFMemory(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public void add(String sessionId, List<Message> messages) {
        try {
            ...
            // 存储到Redis，设置1小时过期时间
            redisTemplate.opsForValue().set(PDF_MEMORY_KEY + sessionId, updatedJson, 1L, TimeUnit.HOURS);
        } catch (Exception e) {
    ...
    public List<Message> get(String sessionId, int lastN) {
        try {
            // 从Redis获取消息
            String jsonMessages = redisTemplate.opsForValue().get(PDF_MEMORY_KEY + sessionId);
