...
import static com.heima.tool.Constants.CHAT_MEMORY_KEY;

/**
 * Redis聊天记忆实现类
 * 实现ChatMemory接口，使用Redis存储和管理聊天消息
 * 支持消息的添加、获取和清空操作
 */
@Repository
public class RedisChatMemory implements ChatMemory {
    ...
    public void add(String sessionId, List<Message> messages) {
        try {
            ...
            // 存储到Redis，设置1小时过期时间
            redisTemplate.opsForValue().set(CHAT_MEMORY_KEY + sessionId, updatedJson, 1L, TimeUnit.HOURS);
        } catch (Exception e) {
    ...
    public List<Message> get(String sessionId, int lastN) {
        try {
            ...
            // 从Redis获取消息
            String jsonMessages = redisTemplate.opsForValue().get(CHAT_MEMORY_KEY + sessionId);
