...
import static com.heima.tool.Constants.SERVICE_MEMORY_KEY;

@Repository
public class RedisServiceMemory implements ChatMemory {

    private StringRedisTemplate redisTemplate;
    private ObjectMapper objectMapper;

    public RedisServiceMemory(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public void add(String sessionId, List<Message> messages) {
        try {
            // 获取现有消息
            List<Message> existingMessages = this.get(sessionId, Integer.MAX_VALUE);
            ...
            // 存储到Redis，设置1小时过期时间
            redisTemplate.opsForValue().set(SERVICE_MEMORY_KEY + sessionId, updatedJson, 1L, TimeUnit.HOURS);
        } catch (Exception e) {
    ...
    public List<Message> get(String sessionId, int lastN) {
        try {
            // 从Redis获取消息
            String jsonMessages = redisTemplate.opsForValue().get(SERVICE_MEMORY_KEY + sessionId);
