...
1. 代码块使用注意事项和细节（对应CodeBlockDetail01.java）
    静态代码块的执行规则
    static 代码块（静态代码块）用于初始化类：随类的加载执行，且 只会执行一次 ；
    普通代码块：每创建一个对象，都会执行一次。
2.类的加载时机类会在以下情况被加载【重点】：
    1) 创建类的对象时（new）；
    2) 创建子类对象实例时，父类也会被加载；
...
public class CodeBlockDetail {
    public static void main(String[] args) {
        ...
        AA aa1 = new AA();
        System.out.println("-----------------------------");
        // 2) 创建子类对象实例时，父类也会被加载，而且父类先加载，子类后加载；
        AA aa2 = new AA();
        // 3) 使用类的静态成员时（静态属性、静态方法）。
        // 4) 使用子类的静态成员时（静态属性、静态方法），父类也会被加载，
        //   而且父类先加载，子类后加载
        System.out.println("-----------------------------");
        System.out.println(Cats.n1);

    }
}

class Animal {
    //静态代码块 用于初始化类：随类的加载执行，且 只会执行一次
    static {
...
class Cats extends Animal {
    ...
    //静态代码块 用于初始化类：随类的加载执行，且 只会执行一次
    static {
        System.out.println("Cats的静态代码块1被执行");
    }

    //普通代码块 在创建对象实例时隐式调用（每创建一个对象执行一次）
    {
        System.out.println("Cats的普通代码块1被执行！");
    }
...
class BB {
    ...
    //普通代码块 在创建对象实例时隐式调用（每创建一个对象执行一次）
    {
        System.out.println("BB的普通代码块1被执行！");
    }
}
